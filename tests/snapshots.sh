...
../tools/snapmgr create $path --label="$( date -d @$oneweek2hr '+%Y%m%d_%H:%M:%S_%z')" --noprompt
echo content4 > $outdir/during.4
../tools/snapmgr create $path --label="$( date -d @$oneweek1hr '+%Y%m%d_%H:%M:%S_%z')" --noprompt
echo content5 > $outdir/during.4
echo content6 > $outdir/during.5
echo content7 > $outdir/during.6
rm -f $outdir/before.2
../tools/snapmgr create $path --label="$( date -d @$oneweek0hr '+%Y%m%d_%H:%M:%S_%z')" --noprompt
rm -f $outdir/during.5
echo content8 > $outdir/before.1
../tools/snapmgr create $path --label="$( date -d @$now3hr '+%Y%m%d_%H:%M:%S_%z')" --noprompt
rm -f $outdir/during.6
../tools/snapmgr create $path --label="$( date -d @$now2hr '+%Y%m%d_%H:%M:%S_%z')" --noprompt
echo content3 > $outdir/current.3
../tools/snapmgr create $path --label="$( date -d @$now1hr '+%Y%m%d_%H:%M:%S_%z')" --noprompt
rm -f $outdir/during.6
